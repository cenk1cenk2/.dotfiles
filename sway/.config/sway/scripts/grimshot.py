...
class Grimshot:
    ...
    def get_active_window_info(self) -> Tuple[str, str]:
        """Get geometry and app_id of the currently active window"""
        try:
            result = self.run_command(["swaymsg", "-t", "get_tree"])
            ...
            if not focused:
                raise GrimshotError("No focused window found")

            rect = focused["rect"]
            geometry = f"{rect['x']},{rect['y']} {rect['width']}x{rect['height']}"
            app_id = focused.get("app_id", "Unknown")

            return geometry, app_id
    ...
    def get_focused_output(self) -> str:
        ...
        try:
            result = self.run_command(["swaymsg", "-t", "get_outputs"])
            ...
            raise GrimshotError("No focused output found")
        except (GrimshotError, json.JSONDecodeError, KeyError) as e:
    ...
    def get_window_geometry(self) -> Optional[str]:
        ...
        try:
            ...
            def get_visible_windows(node, windows=None):
                if windows is None:
                    windows = []
                ...
                for child in node.get("nodes", []) + node.get("floating_nodes", []):
                    get_visible_windows(child, windows)

                return windows

            windows = get_visible_windows(tree)
            window_list = "\n".join(windows)
    ...
    def process_subject(self, subject: str) -> Tuple[str, str, str]:
        ...
        if subject == "area":
            ...
            geometry, app_id = self.get_active_window_info()
            what = f"{app_id} window"
    ...
    def main(self):
        ...
        parser = argparse.ArgumentParser(
            description="Grimshot: a helper for screenshots within sway",
