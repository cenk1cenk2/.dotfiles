...
- `etc/modprobe.d/nvidia-power.conf` — `NVreg_DynamicPowerManagement=0x03` (fine-grained RTD3, default for Ampere+), `NVreg_EnableS0ixPowerManagement=1`, `NVreg_PreserveVideoMemoryAllocations=0` (allow suspend while GPU active), `NVreg_DynamicPowerManagementVideoMemoryThreshold=0` (keep VRAM in self-refresh, never fully power off — trades some idle power for lower RTD3 exit latency).
