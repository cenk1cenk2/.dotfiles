...
_font() {
  ...
  # The <alias> sugar is load-bearing, not cosmetic: fontconfig >= 2.18.3
  # derives an extra target="scan" rule from it that stamps genericfamily
  # onto <family>, which is what outranks 48-guessfamily.conf's guess. The
  # desugared match/edit block only prepends the family and loses to it.
